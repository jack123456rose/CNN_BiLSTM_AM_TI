...
class stock_csv_read:
    ...
    def data_loader(self):
        stock_data = pd.read_csv("C:/Users/USER/JupyterProjects/conv_biLSTM_attention_ti/data/kdd17/price_long_50/"+ self.data ,header=0,index_col="Date")
    ...
    def spliter(self, data):
        ...
        for i in range(len(self.dd)-self.x_frames-self.y_frames+1):
            ...
            y = self.dd.iloc[i +self.x_frames : i +self.x_frames +self.y_frames, 17:]
            xy.append(X)
            xy.append(y)
            data_list.append(xy)
        return data_list
